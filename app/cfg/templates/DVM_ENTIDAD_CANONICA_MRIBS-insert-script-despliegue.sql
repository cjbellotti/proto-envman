	...
	select count(1) into V_COUNT from DTVLA.DVM_ENTIDAD_CANONICA_MRIBS where NOMBRE = {{ VALORES.NOMBRE }};
	counter_ins_tot:=counter_ins_tot+1;
	if V_COUNT = 0 THEN
		select nvl(max(id),0)+1 into entidad{{ VALORES.ID }} from DTVLA.DVM_ENTIDAD_CANONICA_MRIBS;

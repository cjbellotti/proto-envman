EnvMan.Views.EntidadMRIBS = Backbone.View.extend({
	...
	guardar: function () {
		...
		if (!id) {
			...
			var ultima = window.manageData.colecciones['DVM_ENTIDAD_CANONICA_MRIBS'].last();
			if (ultima) {
				id = ultima.get("ID") + 1;
			}
    ...
    if (nombre.length == 0) {
      ...
      window.manageData.colecciones['DVM_ENTIDAD_CANONICA_MRIBS'].add(this.model);

      if (nuevo) {

        //var index = _.findIndex(window.job.registros.entidadcanonica, { NOMBRE : nombre });
        var index = _.findIndex(window.job.registros['DVM_ENTIDAD_CANONICA_MRIBS'], { NOMBRE : nombre });
        if (index < 0) {	
            //window.collections.entidades.add(this.model);
            window.manageData.colecciones['DVM_ENTIDAD_CANONICA_MRIBS'].add(this.model);
            generales.agregarRegistroAlJob('DVM_ENTIDAD_CANONICA_MRIBS', this.model.toJSON());
        }

      } else {
        //window.collections.entidades.set(this.model, { remove : false });
        window.manageData.colecciones['DVM_ENTIDAD_CANONICA_MRIBS'].set(this.model, { remove : false });
        generales.modificarRegistroEnJob('DVM_ENTIDAD_CANONICA_MRIBS', this.model.toJSON());
      }

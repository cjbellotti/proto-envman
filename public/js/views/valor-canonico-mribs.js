EnvMan.Views.ValorCanonicoMRIBS = Backbone.View.extend({
	...
	guardar: function () {
		...
		if (!id) {
			...
			var ultima = window.manageData.colecciones['DVM_VALOR_CANONICO_MRIBS'].last();
			if (ultima) {
				id = ultima.get("ID") + 1;
			}
    ...
    if (valor_canonico.length == 0) {
      ...
      if (nuevo) {
        ...
        var index = _.findIndex(window.job.registros['DVM_VALOR_CANONICO_MRIBS'], { ID_ENTIDAD_CANONICA : id_entidad_canonica,
                                        VALOR_CANONICO : valor_canonico});

        if (index < 0) {

            //window.collections.valoresCanonicos.add(this.model);
            window.manageData.colecciones['DVM_VALOR_CANONICO_MRIBS'].add(this.model);
            //generales.agregarValorCanonicoAJob(this.model.toJSON());
            generales.agregarRegistroAlJob('DVM_VALOR_CANONICO_MRIBS',this.model.toJSON());
        ...
        window.manageData.colecciones['DVM_VALOR_CANONICO_MRIBS'].set(this.model, { remove : false});
        //generales.modificarValorCanonicoEnJob(this.model.toJSON());
        generales.modificarRegistroEnJob('DVM_VALOR_CANONICO_MRIBS', this.model.toJSON());

EnvMan.Views.ValorSistemaMRIBS = Backbone.View.extend({
	...
	initialize : function () {
		...
		this.listenTo(window.manageData.colecciones['DVM_SISTEMA_MRIBS'], 'add', this.renderItemComboSistema);
		this.listenTo(window.manageData.colecciones['DVM_ENTIDAD_CANONICA_MRIBS'], 'add', this.renderItemComboEntidad);
		this.listenTo(window.manageData.colecciones['DVM_VALOR_CANONICO_MRIBS'], 'add', this.renderItemComboValorCanonico);
	...
	guardar: function () {
		...
		if (!id) {
			...
			var ultima = window.manageData.colecciones['DVM_VALOR_SISTEMA_MRIBS'].last();
			if (ultima) {
				id = ultima.get("ID") + 1;
			}
    ...
    if (valor_sistema.length == 0) {
      ...
      if (nuevo) {
        ...
        var index = _.findIndex(window.job.registros['DVM_VALOR_SISTEMA_MRIBS'], { ID_SISTEMA : id_sistema,
        ...
        if (index < 0) {
            ...
            window.manageData.colecciones['DVM_VALOR_SISTEMA_MRIBS'].add(this.model);
            //generales.agregarValorSistemaAJob(this.model.toJSON());
            window.generales.agregarRegistroAlJob('DVM_VALOR_SISTEMA_MRIBS', this.model.toJSON());

        }

      } else {
        //window.collections.valoresSistema.set(this.model, {remove : false});
        window.manageData.colecciones['DVM_VALOR_SISTEMA_MRIBS'].set(this.model, {remove : false});
        //generales.modificarValorSistemaEnJob(this.model.toJSON());
        generales.modificarRegistroEnJob('DVM_VALOR_SISTEMA_MRIBS', this.model.toJSON());
      }
  ...
  cargarComboSistema : function (e) {
    ...
    var self = this;
		...
		window.manageData.colecciones['DVM_SISTEMA_MRIBS'].each(function (sistema) {
    ...
    window.generales.cargarComboPaises(this.$el.find('#pais-sel'), window.job.target, '', function () {
      ...
      if (data['ID_SISTEMA']) {
        ...
        var sistema = window.manageData.colecciones['DVM_SISTEMA_MRIBS'].get(data['ID_SISTEMA']);
		...
		window.manageData.colecciones['DVM_ENTIDAD_CANONICA_MRIBS'].each(function (entidad) {
	...
	cargarComboValorCanonico : function () {
		...
		var valores = window.manageData.colecciones['DVM_VALOR_CANONICO_MRIBS'].where({

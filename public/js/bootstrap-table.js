...
!function ($) {
    ...
    BootstrapTable.prototype.remove = function (params) {
        ...
        var hasIndex = params.hasOwnProperty('indexes');
        if ((!params.hasOwnProperty('field') && !hasIndex) || 
        	(!params.hasOwnProperty('values') && !hasIndex)) {
            return;
        }
        ...
        for (i = len - 1; i >= 0; i--) {
            ...
            if (!row.hasOwnProperty(params.field) && !hasIndex) {
                return;
            }

            if (hasIndex)
            	for (var iIndex = 0; iIndex < params.indexes.length && !match; iIndex++) 
            		if (params.indexes[iIndex] == i)
            			match = true;	            

            if ($.inArray(row[params.field], params.values) !== -1 || match) {
    ...
    BootstrapTable.prototype.getSelections = function () {
        ...
        return $.grep(this.data, function (row, index) {
        	var rowData = row[that.header.stateField];
